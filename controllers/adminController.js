...
exports.postAddFood = async (req, res) => {
    try {
        ...
        res.status(500).send('Sender error while adding food item')
    }
}
...
exports.getEditFood = async (req, res) => {
    ...
    try {
        const foodItem = await db.one('SELECT * FROM food_items WHERE id = $1', [id]);
        res.render('admin/editFood', { food: foodItem });
    } catch (error) {
...
exports.postEditFood = async (req, res) => {
    ...
    try {
        const food = db.oneOrNone('SELECT * FROM food_items WHERE id = $1', [id]);
        if (!food) {
        ...
        res.status(500).send('An error occured while fetching food item');
    }
}
